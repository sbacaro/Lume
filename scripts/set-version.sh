...
#
# Uso:
#   ./scripts/set-version.sh                 # mostra a versão atual
#   ./scripts/set-version.sh 1.3.1           # nova versão; build = atual + 1
#   ./scripts/set-version.sh 1.4.0 7         # nova versão e build explícitos
#
set -euo pipefail
...
if [[ -z "$VER" ]]; then
  ...
  echo "Uso: ./scripts/set-version.sh <versão> [build]"
  exit 0
fi
